...
fn main() {
   ...
   let (train, test) = linfa_datasets::iris()
    ...
    println!("Now let's go on to build and train decision-tree model");
    println!("{:?}", predictions);
...
fn get_dataset() -> Dataset<f32, i32, ndarray::Dim<[usize; 1]>>{
    ...
    let target_index = headers.len()-1; // the last index sure that's where our target field feature is...
